...
void tpm_timer_init(uint_16 TPM_i, uint32_t f, float MOD_Value)
{
	...
	//4. 写TPM_i的状态和控制寄存器
	TPM_ARR[TPM_i]->SC |=
	TPM_SC_TOIE_MASK | TPM_SC_CMOD(1) | TPM_SC_PS(clk_div);
	//5. 计数器清零
	TPM_ARR[TPM_i]->CNT = 0;
	//6. 设置模数寄存器
...
void pwm_init(uint_16 tpmx_Chy, float duty, uint_8 Align, uint_8 pol)
{
	...
	TPM_CnV_REG(TPM_ARR[TPM_i],chl) = (uint_32) (period * duty / PWM_MAX);
}
...
void pwm_update(uint_16 tpmx_Chy, float duty)
{
	...
	TPM_CnV_REG(TPM_ARR[TPM_i],chl) = (uint32_t) period * duty / PWM_MAX;
}

//=====================================================================
//函数名称：pwm_get
//功能概要：获取tpmx模块Chy通道的PWM占空比
//参数说明：tpmx_Chy：模块通道号（例：TPM_CH0表示为TPM0模块0通道）
//函数返回：该通道占空比
//=====================================================================
float pwm_get(uint_16 tpmx_Chy)
{
	uint_8 TPM_i, chl;  //由tpmx_Chy解析出的tpm模块号、通道号临时变量
	uint_32 period;
	float duty;

	//取得引脚复用值，并获得解析的tpm模块号和通道号
	tpm_mux_val(tpmx_Chy, &TPM_i, &chl);

	period = TPM_ARR[TPM_i]->MOD;
	//计算占空比
	duty = (TPM_CnV_REG(TPM_ARR[TPM_i], chl)) * PWM_MAX / period;
	return duty;
}
